...
const readCsv = (filePath) =>
  new Promise((resolve, reject) => {
    ...
    fs.createReadStream(filePath)
      ...
      .on('end', () => {
        if (filePath === productsFile) {
          const mapped = rows.map((r, idx) => ({ ...r, id: String(idx + 1) }));
          resolve(mapped);
        } else {
          resolve(rows);
        }
  ...
  try {
    ...
    for (const item of newItems) {
      ...
      if (idx !== -1) {
        ...
        // Add new item
        currentProducts.push(sanitizedItem);
        addedCount++;
      }
    }
...
// ─── Admin Security API ──────────────────────────────────────
const adminPasswordFile = path.join(__dirname, 'db', 'admin.txt');

app.post('/api/admin/verify', (req, res) => {
  console.log('[POST] /api/admin/verify');
  try {
    if (!fs.existsSync(adminPasswordFile)) {
      return res.status(500).json({ error: '인증 파일이 존재하지 않습니다.' });
    }
    const storedPassword = fs.readFileSync(adminPasswordFile, 'utf8').trim();
