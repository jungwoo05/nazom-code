...
const ProductsTab = () => {
  ...
  const [selectedDetailedModelFilter, setSelectedDetailedModelFilter] = useState('');
  ...
  const filterBrandId = getBrandIdByName(selectedBrandFilter);
  const filterModels = filterBrandId ? getModelsForManufacturer(filterBrandId) : [];
  
  const filterModelId = getModelIdByName(selectedBrandFilter, selectedModelFilter);
  const filterDetailedModels = filterModelId ? getDetailedModelsForModel(filterModelId) : [];
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      setProducts(data);
      setSelectedIds([]); // Reset selection when fetching data
  ...
  const handleDownloadCsv = () => {
    ...
    const headers = ['브랜드', '차종', '연식', '세부모델', 'category', 'name', 'price'];
    const csvRows = [
      headers.join(','),
      ...products.map(p => [
        `"${(p['브랜드'] || '').replace(/"/g, '""')}"`,
  ...
  useEffect(() => {
    setCurrentPage(1);
    setSelectedIds([]);
  }, [searchTerm, selectedBrandFilter, selectedModelFilter, selectedYearFilter, selectedDetailedModelFilter, selectedCategoryFilter]);
  ...
  const filteredProducts = products.filter(p => {
    ...
    if (selectedModelFilter !== '') {
      if (p['차종'] !== selectedModelFilter) return false;
    }
    if (selectedYearFilter !== '') {
      if (String(p['연식']) !== String(selectedYearFilter)) return false;
    }
    if (selectedDetailedModelFilter !== '') {
      if (p['세부모델'] !== selectedDetailedModelFilter) return false;
    }
  ...
  const handleBulkDelete = async () => {
    ...
    const confirmMsg = `선택한 ${selectedIds.length}개의 정비 항목을 정말 삭제하시겠습니까?`;
    if (!window.confirm(confirmMsg)) return;

    setLoading(true);
    try {
      const sortedIdsToDelete = [...selectedIds]
        .map(Number)
        .sort((a, b) => b - a);
      ...
      <div className="tab-toolbar">
        ...
        <div className="toolbar-actions">
          {selectedIds.length > 0 && (
            ...
            >
              <Trash2 size={16}/> 선택 삭제 ({selectedIds.length}개)
            </button>
          )}
          <button className="icon-btn" onClick={fetchData}><RefreshCw size={16}/> 새로고침</button>
        ...
        <div className="filter-selects-wrap">
          <select
            ...
            onChange={e => {
              ...
              setSelectedDetailedModelFilter('');
            }}
          ...
          <select
            ...
            disabled={!selectedModelFilter}
            className="filter-select"
          >
            <option value="">전체 연식</option>
            {getYears().map(y => (
              <option key={y} value={String(y)}>{y}년형</option>
            ))}
          </select>

          <select
            value={selectedDetailedModelFilter}
            onChange={e => setSelectedDetailedModelFilter(e.target.value)}
            disabled={!selectedModelFilter}
            className="filter-select"
          >
            <option value="">전체 세부모델</option>
            {filterDetailedModels.map(m => (
              <option key={m.id} value={m.name}>{m.name}</option>
            ))}
          </select>
          ...
          <thead>
            <tr>
              <th style={{ width: '120px', whiteSpace: 'nowrap' }}>
                <label style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', cursor: 'pointer', color: 'var(--text-tertiary)', fontWeight: '800', userSelect: 'none' }}>
                  <input 
                    type="checkbox" 
  ...
  if (!isAuthenticated) {
    return (
      <div className="admin-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
        <div className="admin-panel lock-panel">
          <div className="admin-header">
            <div className="admin-title">
              <div className="admin-badge security">SECURITY</div>
              <h2>관리자 인증</h2>
            </div>
            <button className="close-btn" onClick={onClose}><X size={22}/></button>
          </div>
          <div className="admin-lock-body">
            <div className={`admin-lock-card ${isShaking ? 'shake' : ''}`}>
              <div className="lock-icon-wrap">
                <Lock size={40} className="lock-icon" />
              </div>
              <h3>관리자 비밀번호 입력</h3>
              <p>접속을 계속하려면 관리자 비밀번호를 입력해주세요.</p>
              
              <form onSubmit={handleVerify} className="lock-form">
                ...
                <button type="submit" className="lock-submit-btn" disabled={verifying}>
                  {verifying ? '인증 진행 중...' : '인증 및 접속'}
                </button>
              </form>
            </div>
          </div>
