...
const CarSelectionWidget = ({ onProceed }) => {
  ...
  const [availableBrands, setAvailableBrands] = useState(manufacturers);
    ...
    fetch('/api/products')
      ...
      .then((products) => {
        const uniqueBrandNames = new Set(
          products
            .map((p) => (p['브랜드'] || p.brand || '').trim())
            .filter(Boolean)
        );
        const filtered = manufacturers.filter((m) => uniqueBrandNames.has(m.name));
        setAvailableBrands(filtered);
      })
      .catch((err) => {
        console.error('Failed to load brands from products', err);
        setAvailableBrands(manufacturers);
      });
  }, []);

  const getOptions = (s) => {
    switch (s) {
      case 0: return availableBrands;
