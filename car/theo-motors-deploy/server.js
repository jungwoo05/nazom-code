...
app.post('/api/reservations', async (req, res) => {
  try {
    ...
    existing.push({ '예약일시': new Date().toLocaleString('ko-KR'), '고객명/연락처': customer, '예약차량상세': car, '선택정비항목': items.join(' | '), '총견적금액': total });
...
app.post('/api/posts', async (req, res) => {
  try {
    ...
    const newPost = { id: String(maxId + 1), date: new Date().toLocaleDateString('ko-KR').replace(/ /g, ''), title: req.body.title, content: req.body.content, isPopup: req.body.isPopup ? 'true' : 'false' };
...
app.put('/api/posts/:id', async (req, res) => {
  try {
    ...
    if (idx === -1) return res.status(404).json({ error: 'Not found' });
    if (req.body.isPopup === true || req.body.isPopup === 'true') posts.forEach(p => p.isPopup = 'false');
    posts[idx] = { ...posts[idx], ...req.body, id: req.params.id, isPopup: req.body.isPopup !== undefined ? String(req.body.isPopup) : posts[idx].isPopup };
    await writePosts(posts);
    res.json(posts[idx]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
// SPA fallback — 모든 미매칭 요청을 index.html로
app.get('*', (req, res) => {
